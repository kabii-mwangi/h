...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add_department' && hasPermission('hr_manager')) {
        ...
        } elseif ($action === 'edit_department' && hasPermission('hr_manager')) {
            $id = $_POST['id'];
            $name = sanitizeInput($_POST['name']);
            $description = sanitizeInput($_POST['description']);
            
            try {
                $stmt = $pdo->prepare("UPDATE departments SET name=?, description=?, updated_at=NOW() WHERE id=?");
        ...
        } elseif ($action === 'add_section' && hasPermission('hr_manager')) {
            $name = sanitizeInput($_POST['section_name']);
            $description = sanitizeInput($_POST['section_description']);
            $department_id = $_POST['department_id'];
            
            try {
                $stmt = $pdo->prepare("INSERT INTO sections (name, description, department_id) VALUES (?, ?, ?)");
        ...
        } elseif ($action === 'edit_section' && hasPermission('hr_manager')) {
            $id = $_POST['section_id'];
            $name = sanitizeInput($_POST['section_name']);
            $description = sanitizeInput($_POST['section_description']);
            $department_id = $_POST['department_id'];
            ...
            try {
                $stmt = $pdo->prepare("UPDATE sections SET name=?, description=?, department_id=?, updated_at=NOW() WHERE id=?");
        ...
        } elseif ($action === 'delete_department' && hasPermission('hr_manager')) {
            $id = $_POST['id'];
            ...
            try {
                ...
                if ($employeeCount > 0) {
                    ...
                    $stmt = $pdo->prepare("DELETE FROM sections WHERE department_id = ?");
                    $stmt->execute([$id]);
        ...
        } elseif ($action === 'delete_section' && hasPermission('hr_manager')) {
            $id = $_POST['id'];
            
            try {
                // Check if section has employees
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM employees WHERE section_id = ?");
                $stmt->execute([$id]);
                $employeeCount = $stmt->fetchColumn();
